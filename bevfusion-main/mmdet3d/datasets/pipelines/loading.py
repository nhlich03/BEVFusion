...
class LoadBEVSegmentation:
    def __init__(
        ...
        # Load maps lazily.  A custom nuScenes-compatible dataset normally
        # ships only the map referenced by its log records; eagerly loading all
        # four official nuScenes locations makes such a dataset fail before the
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if location not in LOCATIONS:
            raise ValueError(
                f"Unsupported nuScenes map location {location!r}; "
                f"expected one of {tuple(LOCATIONS)}"
            )
